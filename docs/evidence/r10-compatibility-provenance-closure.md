# R10 Lane E — Compatibility, Provenance, Rollback, and Supersession Closure Evidence

- Sensitivity: PUBLIC
- Date: 2026-09-08
- Canonical document: `docs/evidence/20260908_r10-compatibility-provenance-closure.md`
- Scope: Lane E (Phase E compatibility, provenance, rollback, supersession, and generalized guard closure; R10-D points D1–D4, 4 points).
- Baseline: HEAD `ff0ab7f1b7bb92975f1e44e2aafabb7f4a1d4e7c` ("docs: retain normalized R10 workflow goal").
- Authority: `docs/evidence/r10-design-freeze.md` (§0–§5, §7, §8, §9 R10-D), `docs/PROVENANCE.md`, `docs/design/20260907_r8plus_execution_plan.md`, `docs/design/20260908_r10_domain-migration-final-acceptance_orchestrator_handoff.md`.
...
Required behaviors are strictly distinguished from legacy-only exclusions and deferred behaviors. No legacy behavior has been silently omitted.
...
1. **Atomic Supersession on Activation**: Under `SkillRegistry.activate`, activating a newer version (e.g. `v2`) atomically transitions any older active version (`v1`) to terminal state `superseded`, invalidates its token, and records `active->superseded:registry-superseded-by-v2:<evidence_id>`.
